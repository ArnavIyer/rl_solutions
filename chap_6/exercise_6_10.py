...
def run_simulation(env_class, alpha, epsilon):
    ...
    while counter < 15000:
        ...
        while not done:
            ...
            episodes.append(ep)
    return timesteps, episodes, policy
...
def plot_optimal_path(env_class, policy):
    env = env_class()
    state = env.reset()
    path_x = [state.x]
    path_y = [state.y]
...
plot_optimal_path(Env4, policy4)
plot_optimal_path(Env8, policy8)
plot_optimal_path(Env9, policy9)